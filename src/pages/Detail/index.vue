          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                />
                <a href="javascript:" class="plus" @click="buyNumber++">+</a>
                <a href="javascript:" class="mins" @click="buyNumber>1 ? buyNumber-- : 1">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  methods: {
    ...
    //表单元素修改购买个数
    changeBuyNumber(e) {
      //收集用户输入的数据并让其乘以1，如果是字符串*1结果就是NaN
    ...
    async addShopCart() {
      //发请求--将产品加入到数据库（通知服务器）
      /*这里派发action其实是调用仓库里的AddOrUpdateShopCart函数，还该函数被async包裹，所以返回结果是一个Promise对象。
      所以此处就可以用await去接这个Promise对象,因此就要在函数名上加上async，
      这样就可以拿到Promise的结果就可以确定本次请求是成功还是失败*/
      try {
        //成功
        await this.$store.dispatch("AddOrUpdateShopCart", {
          skuId: this.$route.params.skuId,
          skuNum: this.buyNumber,
        });
        //因为参数是个对象所以不用query传递，而是将参数放在会话存储中（本地存储占空间，没必要）
        sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
        this.$router.push({name:'addcartsuccess',query:{buyNumber:this.buyNumber}})
      } catch (error) {
        //失败
        alert(error.message);
      }
